...
Repository root: C:\Users\KIIT0001\blogs
Blind packet: C:\Users\KIIT0001\blogs\.desloppify\review_packet_blind.json
Batch index: 17
...
Overview (12 signals):
  design_concern: 5 — builder/cache/cache_reads.go, builder/run/build.go, ...
  duplication_design: 5 — builder/cache/cache_writes.go, builder/renderer/native/d2.go, ...
  mixed_responsibilities: 2 — builder/renderer/renderer.go, builder/testutil/helpers.go
...
For each concern, read the source code and report your verdict in issues[]:
  ...
  - [design_concern] builder/cache/cache_reads.go
    ...
    evidence: [signature] 'GetPostsByIDs' has 2 different signatures across 4 files
    fingerprint: cbebb79650587d50
  - [design_concern] builder/run/build.go
    summary: Design signals from structural
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: structural
    evidence: File size: 582 lines
    fingerprint: f50cbcc4887166df
  - [design_concern] builder/run/incremental.go
    summary: Design signals from structural
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: structural
    evidence: File size: 594 lines
    fingerprint: 27cc5940225c650f
  - [design_concern] builder/search/engine.go
    summary: Design signals from structural
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: structural
    evidence: File size: 620 lines
    fingerprint: f4498077dcdc0914
  - [design_concern] builder/utils/fs_copy.go
    summary: Design signals from structural
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: structural
    evidence: File size: 592 lines
    fingerprint: d76670352f7f0ebe
  - [duplication_design] builder/cache/cache_writes.go
    ...
    evidence: Flagged by: boilerplate_duplication, signature
    evidence: [signature] 'BatchCommit' has 2 different signatures across 4 files
    fingerprint: 73652e9b991a6e15
  - [duplication_design] builder/renderer/native/d2.go
    summary: Duplication pattern — assess if extraction is warranted
    question: Is the duplication worth extracting into a shared utility, or is it intentional variation?
    evidence: Flagged by: boilerplate_duplication
    evidence: [boilerplate_duplication] Boilerplate block repeated across 2 files (window 16 lines): builder/renderer/native/d2.go:27, builder/renderer/native/math.go:98
    fingerprint: f65c4045ab3b416b
  - [duplication_design] builder/services/mocks/sink_mock.go
    summary: Duplication pattern — assess if extraction is warranted
    question: Is the duplication worth extracting into a shared utility, or is it intentional variation?
    evidence: Flagged by: boilerplate_duplication
    evidence: [boilerplate_duplication] Boilerplate block repeated across 2 files (window 89 lines): builder/services/mocks/sink_mock.go:3, builder/testutil/sink_mock.go:3
    fingerprint: 97bbade02f116d94
  - [duplication_design] builder/services/post_service.go
    summary: Duplication pattern — assess if extraction is warranted
    question: Is the duplication worth extracting into a shared utility, or is it intentional variation?
    evidence: Flagged by: boilerplate_duplication
    evidence: [boilerplate_duplication] Boilerplate block repeated across 2 files (window 23 lines): builder/services/post_service.go:215, builder/services/post_single.go:102
    fingerprint: 04ae625604ffe93f
  - [duplication_design] builder/utils/copy_fallback.go
    summary: Duplication pattern — assess if extraction is warranted
    question: Is the duplication worth extracting into a shared utility, or is it intentional variation?
    evidence: Flagged by: boilerplate_duplication
    evidence: [boilerplate_duplication] Boilerplate block repeated across 2 files (window 14 lines): builder/utils/copy_fallback.go:11, builder/utils/copy_optimized_linux.go:13
    fingerprint: 6dde3c77a72c5fed
  - [mixed_responsibilities] builder/renderer/renderer.go
    summary: Issues from 1 detectors — may have too many responsibilities
    question: What are the distinct responsibilities? Would splitting produce modules with multiple independent consumers, or would extracted files only be imported by the parent? Only split if the extracted code would be reused.
    evidence: Flagged by: responsibility_cohesion
    evidence: [responsibility_cohesion] 9 disconnected function clusters (13 functions) — likely mixed responsibilities
    fingerprint: ecb0a79346e803b7
  - [mixed_responsibilities] builder/testutil/helpers.go
    summary: Issues from 1 detectors — may have too many responsibilities
    question: What are the distinct responsibilities? Would splitting produce modules with multiple independent consumers, or would extracted files only be imported by the parent? Only split if the extracted code would be reused.
    evidence: Flagged by: responsibility_cohesion
    evidence: [responsibility_cohesion] 8 disconnected function clusters (11 functions) — likely mixed responsibilities
    fingerprint: a89a4ad9ff7d2f42
...
then read the actual source code.
  ...
  desloppify show boilerplate_duplication --no-budget      # 5 findings
  desloppify show responsibility_cohesion --no-budget      # 2 findings
  desloppify show signature --no-budget      # 4 findings
  desloppify show structural --no-budget      # 4 findings
  desloppify show unused --no-budget      # 10 findings
