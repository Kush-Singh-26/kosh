...
type syncContext struct {
	...
	siteFiles, themeFiles, siteEnqueued, themeEnqueued int64
	relErrs int64
	siteSamples, themeSamples []string
	sampleMu sync.Mutex
}
...
func (s *assetService) syncStaticAssets(ctx context.Context, bgCtx context.Context, skipImages bool) (imageQueue []imageCopyTask, err error) {
	...
	walkFunc := s.setupDiscoveryWalk(ctx, &walkerWg, walkConcurrency, debugAssets, sc, enqueue)
...
func (s *assetService) setupDiscoveryWalk(ctx context.Context, walkerWg *sync.WaitGroup, walkConcurrency int, debugAssets bool, sc *syncContext, enqueue func(assetTask)) func(context.Context, string, string, bool) error {
	return func(ctx context.Context, dir, label string, isSite bool) error {
		...
		go func() {
			...
			_ = fspkg.ParallelWalk(fspkg.WalkOptions{
				...
				WalkFn: func(path string, info fs.FileInfo, err error) error {
					...
					if debugAssets {
						if isSite {
							atomic.AddInt64(&sc.siteFiles, 1)
						} else {
							atomic.AddInt64(&sc.themeFiles, 1)
						}
					}
					...
					if relErr != nil || rel == "" {
						rel = s.handleRelPathManualFallback(dir, path, debugAssets, sc)
						if rel == "" {
							return nil
						}
					}
					fullRel := "static/" + rel
					if _, loaded := sc.seen.LoadOrStore(fullRel, true); !loaded {
						if debugAssets {
							s.recordDiscoverySample(isSite, fullRel, sc)
						}
