// Package sink provides artifact sink interfaces for streaming file writes during builds.
package sink
...
func NewDiskSink(stagingDir, realOutputDir string) *DiskSink {
	sDir, err := utils.AbsNormalizePath(stagingDir)
	if err != nil {
		sDir = utils.NormalizePath(stagingDir)
	}
	rDir, err := utils.AbsNormalizePath(realOutputDir)
	if err != nil {
		rDir = utils.NormalizePath(realOutputDir)
	}
	s := &DiskSink{
		stagingDir:         sDir,
		realOutputDir:      rDir,
...
func (s *DiskSink) resolvePathForWrite(p string) (string, error) {
	...
	if filepath.IsAbs(filepath.FromSlash(cleanP)) {
		...
		resolvedAbs = utils.NormalizePath(resolvedAbs)
		...
		if isInsideDir(resolvedAbs, s.realOutputDirLower) {
			...
			resolved = s.fastJoinStaging(utils.NormalizePath(rel))
...
func (s *DiskSink) Register(p string) {
	...
	if !filepath.IsAbs(filepath.FromSlash(cleanP)) {
		finalPath = utils.NormalizePath(filepath.Join(filepath.FromSlash(s.realOutputDir), filepath.FromSlash(cleanP)))
	} else {
		if hasPrefixCaseInsensitive(cleanP, s.stagingDirLower) {
			...
			finalPath = utils.NormalizePath(filepath.Join(filepath.FromSlash(s.realOutputDir), filepath.FromSlash(rel)))
		} else {
			finalPath = cleanP
		}
	}
...
func (s *DiskSink) CopyFile(src, dst string) error {
	...
	if err := utils.CopyFileInternal(src, filepath.FromSlash(target)); err != nil {
		return err
	}
