...
func TestWorkerPool_SchedulerTokenReleaseOnCancel(t *testing.T) {
	...
	mockScheduler := &mockScheduler{
		acquire: func(ctx context.Context, task utils.TaskType) error {
			acquired.Add(1)
			// Block until context is cancelled
			<-ctx.Done()
			return ctx.Err()
		},
		release: func(task utils.TaskType) {
			released.Add(1)
		},
	}
	...
	pool.WithScheduler(mockScheduler, utils.TaskDefault)
	pool.Start()
...
func (m *mockScheduler) Acquire(ctx context.Context, task utils.TaskType) error {
	if m.acquire != nil {
...
func (m *mockScheduler) Release(task utils.TaskType) {
	if m.release != nil {
		m.release(task)
	}
}
