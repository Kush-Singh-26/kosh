...
func CopyFileWithOptionalImageProcessing(opts ProcessImageOptions) error {
	...
	if opts.Opts.Compress && isImage {
		...
		if err := processImageVFS(newOpts); err != nil {
			return err
		}
			...
			for {
				...
				case task, ok := <-imageQueue:
					...
					func() {
						...
						if err := processImageVFS(ProcessImageOptions{
							Ctx:     ctx,
							SrcFs:   srcFs,
							Sink:    sink,
							SrcPath: task.path,
...
func processImageVFS(opts ProcessImageOptions) error {
	if opts.SrcInfo == nil {
		var err error
	...
	if err := utils.GlobalScheduler.Acquire(opts.Ctx, utils.TaskImage); err != nil {
		return err
	}
	defer utils.GlobalScheduler.Release(utils.TaskImage)
	...
	br := utils.SharedBufioReaderPool.Get(f)
	defer func() {
		br.Reset(nil)
		utils.SharedBufioReaderPool.Put(br)
	}()
