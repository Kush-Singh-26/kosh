...
func TestRunMigrations_NoOp(t *testing.T) {
	...
	newVer, err := RunMigrations(db, uint32(core.SchemaVersion), slog.Default())
	if err != nil {
		t.Fatalf("RunMigrations failed: %v", err)
	}

	if newVer != uint32(core.SchemaVersion) {
		t.Errorf("Expected version to remain %d, got %d", core.SchemaVersion, newVer)
	}
}
