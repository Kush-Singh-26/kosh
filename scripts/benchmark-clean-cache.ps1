param(
    ...
    [int]$RunsPerCombo = 3,
    [string]$ResultsFile = ".\docs\build-benchmark-results.md",
    [string]$ConfigFile = "C:\Users\KIIT0001\Kush-Singh-26.github.io\blogs-src\kosh.yaml",
    [string]$LogsDir = ".\perf-results\benchmark-logs"
)
...
function Upsert-ResultRow {
    ...
    $row = "| $VipsConcurrency | $ImageWorkers | $Run | $Total | $RootStatic | $AssetBuilding | $ParsePosts | $Notes |"
    ...
    foreach ($vips in $VipsConcurrencyValues) {
        foreach ($workers in $ImageWorkerValues) {
            for ($run = 1; $run -le $RunsPerCombo; $run++) {
                Write-Host "Running benchmark: vipsConcurrency=$vips imageWorkers=$workers run=$run" -ForegroundColor Cyan
                ...
                $logFile = Join-Path $LogsDir ("bench-v{0}-w{1}-run{2}-{3}.log" -f $vips, $workers, $run, $timestamp)
                $stdoutFile = Join-Path $LogsDir ("bench-v{0}-w{1}-run{2}-{3}.stdout.log" -f $vips, $workers, $run, $timestamp)
                $stderrFile = Join-Path $LogsDir ("bench-v{0}-w{1}-run{2}-{3}.stderr.log" -f $vips, $workers, $run, $timestamp)

                $proc = Start-Process -FilePath "kosh" -ArgumentList "clean", "--cache" -WorkingDirectory $siteWorkingDir -NoNewWindow -PassThru -Wait -RedirectStandardOutput $stdoutFile -RedirectStandardError $stderrFile
                ...
                $total = Parse-Metric -Text $output -Pattern "Built 39 posts in (.*?) \(cache:"
                if ($total -eq "N/A") {
                ...
                Upsert-ResultRow -FilePath $ResultsFile -VipsConcurrency $vips -ImageWorkers $workers -Run $run -Total $total -RootStatic $rootStatic -AssetBuilding $assetBuilding -ParsePosts $parsePosts -Notes $notes
            }
        }
    }
}
...
Write-Host "Benchmarking complete. Results written to $ResultsFile" -ForegroundColor Green
